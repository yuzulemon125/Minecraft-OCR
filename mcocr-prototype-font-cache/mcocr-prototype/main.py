...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("image", nargs="?", help="PNG screenshot or cropped text image")
    parser.add_argument(
        "--config", default="config.json",
        help="configuration file (default: config.json)",
    )
    parser.add_argument(
        "--gui-scale", type=int, default=None,
        help="override gui_scale in config.json",
    )
    parser.add_argument(
        "--font-data", default=None,
        help="explicitly use another compiled .npz font data file",
    )
    parser.add_argument(
        "--source-profile", default=None,
        help="explicitly bypass compiled data and read an atlas/profile JSON",
    )
    parser.add_argument(
...
def main() -> int:
    ...
    try:
        ...
        if args.verbose:
            ...
            print(f"grid_phase={result.grid_phase[0]},{result.grid_phase[1]}", file=sys.stderr)
