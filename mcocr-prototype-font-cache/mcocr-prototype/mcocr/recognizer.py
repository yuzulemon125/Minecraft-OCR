...
from .profile import FontProfile, Glyph, load_compiled_profile, load_source_profile
...
class MinecraftOCR:
    def __init__(
        ...
        font_data_path: str | Path | None = None,
        ...
        # Explicit source_profile_path bypasses compiled data for testing/rebuilds.
        if source_profile_path is not None:
            source = Path(source_profile_path)
            if not source.is_absolute():
                source = (self.config_path.parent / source).resolve()
            self.profile = load_source_profile(source)
            self.profile_source = f"source:{source}"
        else:
            data_value = Path(font_data_path or self.config["font_data"])
            if not data_value.is_absolute():
                data_value = (self.config_path.parent / data_value).resolve()
            self.profile = load_compiled_profile(data_value)
            self.profile_source = f"compiled:{data_value}"
    ...
    def check(self) -> list[str]:
        messages = [
            ...
            f"profile source: {self.profile_source}",
